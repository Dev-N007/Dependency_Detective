...
export const WhatIfView: React.FC<WhatIfViewProps> = ({
  ...
  return (
    <div className="flex-1 bg-slate-50 dark:bg-[#080a0f] overflow-y-auto p-6 space-y-6 transition-colors">
      {/* Header Banner */}
      <div className="cyber-panel p-6 rounded-2xl relative overflow-hidden bg-white dark:bg-[#0f141d]/90 border border-slate-200 dark:border-[#202b3c] shadow-xs">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 relative z-10">
          <div>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-xs font-mono px-2.5 py-0.5 rounded-full bg-sky-50 dark:bg-cyan-950 text-sky-700 dark:text-cyan-400 border border-sky-200 dark:border-cyan-500/30 uppercase tracking-wider font-semibold">
            ...
            <h2 className="text-2xl sm:text-3xl font-extrabold font-mono text-slate-900 dark:text-slate-100 flex items-center gap-3">
              ...
              <span className="text-sm font-normal text-slate-500 dark:text-slate-400 font-mono">
                {report.current_version} <ArrowRight className="inline w-3.5 h-3.5 text-sky-600 dark:text-cyan-400" /> {targetVerInput}
              </span>
            </h2>
          </div>
          ...
          <div className="flex items-center gap-4">
            <div
              className={`p-4 rounded-xl border flex items-center gap-3 ${
                report.risk_level === 'HIGH'
                  ? 'bg-rose-50 dark:bg-amber-950/40 border-rose-200 dark:border-amber-500/50 glow-amber text-rose-700 dark:text-amber-400'
                  : report.risk_level === 'MEDIUM'
                  ? 'bg-amber-50 dark:bg-cyan-950/40 border-amber-200 dark:border-cyan-500/50 glow-cyan text-amber-700 dark:text-cyan-300'
                  : 'bg-emerald-50 dark:bg-emerald-950/40 border-emerald-200 dark:border-emerald-500/50 glow-emerald text-emerald-700 dark:text-emerald-400'
              }`}
            >
            ...
            <button
              ...
              className="px-4 py-2.5 rounded-xl bg-white dark:bg-[#151b26] border border-slate-200 dark:border-[#202b3c] hover:border-slate-300 dark:hover:border-slate-500 text-xs font-mono text-slate-700 dark:text-slate-300 transition-colors cursor-pointer shadow-xs font-semibold"
            >
              ← Back to Graph
            </button>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="p-4 rounded-xl bg-white dark:bg-[#0f141d] border border-slate-200 dark:border-[#202b3c] shadow-xs">
        ...
        <div className="p-4 rounded-xl bg-white dark:bg-[#0f141d] border border-slate-200 dark:border-[#202b3c] shadow-xs">
          ...
          <span className="text-2xl font-bold font-mono text-purple-600 dark:text-purple-400">{report.transitive_files_affected}</span>
        </div>
        <div className="p-4 rounded-xl bg-white dark:bg-[#0f141d] border border-slate-200 dark:border-[#202b3c] shadow-xs">
      ...
      <div className="p-5 rounded-2xl bg-white dark:bg-[#0f141d] border border-slate-200 dark:border-[#202b3c] space-y-3 shadow-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="space-y-3">
            {report.affected_files.map((file, idx) => (
              <div
                key={idx}
                className="p-4 rounded-xl bg-white dark:bg-[#0f141d] border border-slate-200 dark:border-[#202b3c] flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-xs"
              >
                <div>
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-[10px] font-mono px-2 py-0.5 rounded border uppercase font-semibold ${
                        file.usage_level === 'HIGH'
                          ? 'bg-rose-50 dark:bg-amber-950/60 border-rose-200 dark:border-amber-500/30 text-rose-700 dark:text-amber-400'
                    ...
                    {file.is_test_file && (
                      <span className="text-[10px] font-mono px-2 py-0.5 rounded bg-sky-50 dark:bg-cyan-950/60 border border-sky-200 dark:border-cyan-500/30 text-sky-700 dark:text-cyan-400 uppercase flex items-center gap-1 font-semibold">
                ...
                {report.evidence.find((ev) => ev.file_path === file.file_path) && (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg bg-sky-50 dark:bg-cyan-950/60 hover:bg-sky-100 dark:hover:bg-cyan-900/60 border border-sky-200 dark:border-cyan-500/30 text-sky-700 dark:text-cyan-300 text-xs font-mono flex items-center gap-1.5 transition-colors whitespace-nowrap cursor-pointer font-semibold"
                  >
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-2xl bg-white dark:bg-[#0f141d] border border-slate-200 dark:border-[#202b3c] space-y-3 shadow-xs">
            {report.recommended_actions.map((act, idx) => (
              <div key={idx} className="flex items-start gap-3 p-3 rounded-lg bg-slate-50 dark:bg-[#151b26] border border-slate-200 dark:border-[#202b3c]">
                <span className="w-5 h-5 rounded-full bg-sky-100 dark:bg-cyan-950 text-sky-700 dark:text-cyan-400 border border-sky-200 dark:border-cyan-500/30 flex items-center justify-center text-xs font-mono font-bold flex-shrink-0">
                  {idx + 1}
                </span>
