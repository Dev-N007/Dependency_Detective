...
export const DependencyGraphView: React.FC<DependencyGraphProps> = ({
  ...
  return (
    <div className="flex-1 flex flex-col lg:flex-row h-full overflow-hidden bg-slate-50 dark:bg-[#080a0f] transition-colors">
      ...
      <div className="flex-1 flex flex-col border-r border-slate-200 dark:border-[#202b3c] p-6 overflow-y-auto cyber-grid">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6 pb-4 border-b border-slate-200 dark:border-[#202b3c]">
          <div className="relative flex-1 max-w-md">
            <Search className="w-4 h-4 text-sky-500 dark:text-cyan-400 absolute left-3 top-3" />
            <input
              type="text"
              placeholder="Search package (e.g. axios, lodash, requests)..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full bg-white dark:bg-[#0f141d] border border-slate-200 dark:border-[#202b3c] rounded-xl pl-9 pr-4 py-2 text-sm text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-sky-500 dark:focus:border-cyan-500 font-mono shadow-xs"
            />
          </div>
            ...
            <div className="flex bg-white dark:bg-[#0f141d] p-1 rounded-xl border border-slate-200 dark:border-[#202b3c] text-xs font-mono shadow-xs">
              <button
                ...
                className={`px-3 py-1 rounded-lg transition-colors ${
                  filterType === 'all'
                    ? 'bg-sky-100 dark:bg-cyan-950 text-sky-700 dark:text-cyan-400 font-bold'
              ...
              <button
                ...
                className={`px-3 py-1 rounded-lg transition-colors ${
                  filterType === 'transitive'
                    ? 'bg-indigo-100 dark:bg-purple-950 text-indigo-700 dark:text-purple-400 font-bold'
                    : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'
                }`}
              >
                Transitive
            ...
            return (
              <div
                ...
                className={`p-4 rounded-2xl border transition-all cursor-pointer relative overflow-hidden group ${
                  isSelected
                    ? 'bg-white dark:bg-[#151b26] border-sky-500 dark:border-cyan-500 shadow-xl shadow-sky-500/10 dark:shadow-cyan-950/40 glow-cyan'
                    : 'bg-white/90 dark:bg-[#0f141d]/90 border-slate-200 dark:border-[#202b3c] hover:border-slate-400 dark:hover:border-slate-600 hover:bg-slate-50 dark:hover:bg-[#151b26]/70 shadow-xs'
                }`}
              >
                ...
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500 dark:text-slate-400 font-mono">v{node.version}</span>
                  </div>
                  <span
                    className={`text-[10px] font-mono uppercase px-2 py-0.5 rounded border ${
                      node.dep_type === 'direct'
                        ...
                        : 'bg-indigo-50 dark:bg-purple-950/60 border-indigo-200 dark:border-purple-500/30 text-indigo-700 dark:text-purple-400'
                    }`}
                  >
                    {node.dep_type}
                  </span>
                ...
                <div className="grid grid-cols-3 gap-2 mt-4 pt-3 border-t border-slate-100 dark:border-[#202b3c]/60 text-xs font-mono">
                  <div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block">Usages</span>
                    <span className="font-semibold text-sky-600 dark:text-cyan-300">{node.usages_count}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block">Direct Dep.</span>
                    <span className="font-semibold text-emerald-600 dark:text-emerald-300">{node.direct_dependents_count}</span>
                  </div>
                  <div>
                    <span className="text-[10px] text-slate-500 dark:text-slate-400 block">Transitive</span>
                    <span className="font-semibold text-indigo-600 dark:text-purple-300">{node.transitive_dependents_count}</span>
                  </div>
                </div>

                {node.is_removable && (
                  <div className="mt-3 pt-2 text-[11px] text-amber-600 dark:text-amber-400 font-mono flex items-center gap-1.5">
      ...
      {selectedNode ? (
        <div className="w-full lg:w-96 bg-white dark:bg-[#0f141d] p-6 border-l border-slate-200 dark:border-[#202b3c] flex flex-col justify-between overflow-y-auto transition-colors">
          <div>
            <div className="flex items-center justify-between mb-4 pb-4 border-b border-slate-200 dark:border-[#202b3c]">
              <div>
                ...
                <h3 className="text-xl font-bold font-mono text-slate-900 dark:text-slate-100">{selectedNode.name}</h3>
              </div>
              <span className="text-xs font-mono px-2.5 py-1 rounded bg-slate-100 dark:bg-[#151b26] border border-slate-200 dark:border-[#202b3c] text-slate-700 dark:text-slate-300">
            ...
            <div className="space-y-4">
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#151b26] border border-slate-200 dark:border-[#202b3c]">
                <span className="text-xs font-mono text-slate-500 dark:text-slate-400 block mb-1">Ecosystem & Scope</span>
                <div className="flex items-center gap-2 text-xs font-mono">
                  <span className="uppercase px-2 py-0.5 rounded bg-sky-100 dark:bg-cyan-950 text-sky-700 dark:text-cyan-400 border border-sky-300 dark:border-cyan-500/30 font-bold">
              ...
              <div className="p-3.5 rounded-xl bg-slate-50 dark:bg-[#151b26] border border-slate-200 dark:border-[#202b3c]">
                <span className="text-xs font-mono text-slate-500 dark:text-slate-400 block mb-2 font-bold">Codebase Impact Profile</span>
                <div className="space-y-2 text-xs font-mono">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="text-indigo-600 dark:text-purple-400 font-bold">{selectedNode.transitive_dependents_count}</span>
                  </div>
                  <div className="flex justify-between">
          ...
          <div className="mt-6 pt-4 border-t border-slate-200 dark:border-[#202b3c] space-y-3">
            <button
              ...
              className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-sky-600 to-sky-500 dark:from-cyan-600 dark:to-cyan-500 hover:from-sky-500 hover:to-sky-400 dark:hover:from-cyan-500 dark:hover:to-cyan-400 text-white dark:text-slate-950 font-bold font-mono text-sm flex items-center justify-center gap-2 shadow-lg shadow-sky-500/20 dark:shadow-cyan-950/50 transition-all group cursor-pointer"
            >
            ...
            <button
              ...
              className="w-full py-3 px-4 rounded-xl bg-slate-100 dark:bg-[#151b26] hover:bg-slate-200 dark:hover:bg-[#1a2332] border border-slate-300 dark:border-[#202b3c] hover:border-slate-400 dark:hover:border-slate-500 text-slate-800 dark:text-slate-200 font-bold font-mono text-sm flex items-center justify-center gap-2 transition-all cursor-pointer shadow-xs"
            >
              <History className="w-4 h-4 text-indigo-600 dark:text-purple-400" />
        ...
        <div className="w-full lg:w-96 bg-white dark:bg-[#0f141d] p-6 border-l border-slate-200 dark:border-[#202b3c] flex items-center justify-center text-center transition-colors">
          <div className="space-y-2 max-w-xs">
            ...
            <h4 className="text-sm font-bold font-mono text-slate-700 dark:text-slate-300">Select a Dependency Node</h4>
