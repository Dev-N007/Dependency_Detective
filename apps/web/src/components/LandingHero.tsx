  ...
  return (
    <div className="flex-1 flex flex-col justify-center items-center px-6 py-12 bg-slate-50 dark:bg-[#080a0f] cyber-grid relative overflow-hidden transition-colors">
      ...
      <div className="max-w-4xl text-center space-y-8 relative z-10">
        {/* Hackathon Badge */}
        <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white dark:bg-[#151b26] border border-slate-200 dark:border-[#202b3c] text-xs font-mono text-sky-600 dark:text-cyan-400 shadow-sm">
        ...
        <div className="space-y-4">
          <h1 className="text-4xl sm:text-6xl font-extrabold tracking-tight font-mono text-slate-900 dark:text-slate-100 leading-tight">
            YOU ARE ABOUT TO CHANGE A DEPENDENCY.{' '}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-600 via-emerald-600 to-purple-600 dark:from-cyan-400 dark:via-emerald-400 dark:to-purple-400 block mt-2">
              DO YOU KNOW WHAT BREAKS?
            </span>
          </h1>

          <p className="text-lg sm:text-xl font-mono text-slate-700 dark:text-slate-300 max-w-2xl mx-auto">
        ...
        <div className="space-y-4 max-w-xl mx-auto">
          <button
            onClick={onLoadDemo}
            disabled={isLoading}
            className="w-full py-4 px-6 rounded-2xl bg-gradient-to-r from-sky-500 via-sky-400 to-emerald-400 dark:from-cyan-500 dark:via-cyan-400 dark:to-emerald-400 hover:from-sky-400 hover:to-emerald-300 text-slate-950 font-extrabold font-mono text-base flex items-center justify-center gap-3 shadow-xl shadow-sky-500/20 dark:shadow-cyan-950/50 transition-all transform hover:-translate-y-0.5 group cursor-pointer"
          >
            {isLoading ? (
          ...
          <div className="relative flex items-center my-4">
            ...
            <span className="flex-shrink mx-4 text-xs font-mono text-slate-500 dark:text-slate-400 uppercase font-bold">OR SCAN LOCAL PATH / GIT REMOTE URL</span>
            <div className="flex-grow border-t border-slate-200 dark:border-[#202b3c]"></div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="flex gap-2">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full bg-white dark:bg-[#0f141d] border border-slate-300 dark:border-[#202b3c] rounded-xl pl-10 pr-4 py-3 text-xs font-mono text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:outline-none focus:border-sky-500 dark:focus:border-cyan-500 shadow-xs"
              />
            </div>
            <button
              type="submit"
              disabled={!customPath.trim() || isLoading}
              className="px-5 py-3 rounded-xl bg-slate-900 dark:bg-[#151b26] border border-slate-700 dark:border-[#202b3c] hover:border-slate-500 text-xs font-mono font-bold text-slate-100 dark:text-slate-200 transition-colors disabled:opacity-50 shadow-xs cursor-pointer"
            >
              Analyze
            </button>
          </form>
          ...
          <div className="flex flex-wrap items-center justify-center gap-2 pt-2 text-xs font-mono">
            ...
            {['expressjs/express', 'axios/axios', 'psf/requests', 'lodash/lodash'].map((preset) => (
              <button
                ...
                disabled={isLoading}
                className="px-2.5 py-1 rounded-lg bg-sky-100 dark:bg-cyan-950/60 hover:bg-sky-200 dark:hover:bg-cyan-900/60 border border-sky-300 dark:border-cyan-500/30 text-sky-700 dark:text-cyan-300 text-[11px] font-mono transition-colors cursor-pointer"
              >
                github.com/{preset}
              </button>
            ))}
        ...
        <div className="pt-8 border-t border-slate-200 dark:border-[#202b3c]/60 grid grid-cols-1 sm:grid-cols-3 gap-4 text-left max-w-3xl mx-auto">
          <div className="p-4 rounded-xl bg-white dark:bg-[#0f141d]/80 border border-slate-200 dark:border-[#202b3c] space-y-1 shadow-xs">
            <div className="text-xs font-mono font-bold text-sky-600 dark:text-cyan-400 flex items-center gap-1.5">
          ...
          <div className="p-4 rounded-xl bg-white dark:bg-[#0f141d]/80 border border-slate-200 dark:border-[#202b3c] space-y-1 shadow-xs">
            <div className="text-xs font-mono font-bold text-purple-600 dark:text-purple-400 flex items-center gap-1.5">
